...
class SinaDownLoader(object):
    ...
    def down_min_kline(self):
        ...
        filename = '%s/%s_%s.csv' % (self.filepath, self.symbol, self.period)
    ...
    def down_day_kline(self):
        ...
        data = pandas.DataFrame(response[::-1], columns=list(self.fields.keys()))
        filename = '%s/%s_day.csv' % (self.filepath, self.symbol)
        data.to_csv(filename)
        return filename
...
if __name__ == '__main__':
    SinaDownLoader('RB2201', '30m', '../../datas').run()
