...
class SF38ShortStrategy(Strategy):
    ...
    def next(self):
        ...
        ll = today_open - bar.atr * self.params.hh_mul
        ...
        if not short_position.volume and bar.close <= ll and self.trend[-1] == -1 and bar.close <= st_line:
            self.sell(self.params.symbol, self.params.lots)
        ...
        if short_position.volume:
            if self.open_bar == 0:
                self.update_array(self.short_exit_price, st_line + bar.atr * self.params.exit_mul)
                self.open_bar += 1
                ...
                if bar.high >= self.short_exit_price[-1]:
                    # max(bar.close, self.short_exit_price[-1])
                    self.close(self.params.symbol, self.broker.Position.Short, self.params.lots)
